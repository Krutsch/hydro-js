...
async function run(args: string[], warmups = "3", repeats = "10") {
  ...
  test(`${mode} captures one marked context after fresh-context warmups`, async () => {
    ...
    try {
      ...
      if (process.platform === "linux") {
        ...
        assert.equal(
          report.metadata.cpuAffinity,
          status.match(/^Cpus_allowed_list:\s*(.+)$/m)?.[1],
        );
      }
      assert.equal(report.profileOutput, output);
      assert.ok(report.result.frameElapsedMs >= report.result.elapsedMs);
      assert.deepEqual(
        JSON.parse(await readFile(report.metadataOutput, "utf8")),
        report,
      );
      ...
      const { traceEvents } = JSON.parse(
        await readFile(report.traceOutput, "utf8"),
      );
      const marks = ["handler-start", "handler-end", "frame-end"].map(
        (name) => {
          const matches = traceEvents.filter(
            (event: { name: string }) => event.name === `hydro-actual-${name}`,
          );
      ...
      assert.ok(
        Math.abs((marks[1].ts - marks[0].ts) / 1000 - report.result.elapsedMs) <
          5,
      );
      assert.ok(
        Math.abs(
          (marks[2].ts - marks[0].ts) / 1000 - report.result.frameElapsedMs,
        ) < 5,
      );
...
test("profiling rejects invalid arguments before capture", async () => {
  ...
  for (const [args, message] of cases) await assert.rejects(run(args), message);
  await assert.rejects(
    run(
      ["create", "--profile", "cpu", "--profile-output", "unused.json"],
      "-1",
    ),
    /ACTUAL_BENCHMARK_WARMUPS/,
  );
});
