...
function addEventListener(
  ...
  if (events) {
    ...
    else if (current !== handler) events.set(eventName, new Set([current, handler]));
  } else {
...
function bindEvent(
  ...
  if (previousHandler) removeTrackedEventListener(node, eventName, previousHandler);
...
function writeTwoWayValue(
  ...
  if (node instanceof window.HTMLInputElement && node.type === Placeholder.radio) {
...
const selectorOwners = new WeakMap<
  reactiveObject<any>,
  { map: Map<any, reactiveObject<boolean>>; key: any }
>();
...
function selector(reactiveHydro: reactiveObject<any>): selectorFn {
  const perKey = new Map<any, reactiveObject<boolean>>();
  let current: any = getValue(reactiveHydro);
  let disposed = false;

  const onSourceChange = (newVal: any) => {
    if (newVal === null) {
      const prev = perKey.get(current);
      if (prev && getValue(prev)) prev(false);
      current = null;
      return;
    }
    ...
    if (next && !getValue(next)) next(true);
    current = newVal;
  };
  ...
  const arm = () => {
    if (!disposed) observe(reactiveHydro, onSourceChange);
  };
  const stopObserving = observe(reactiveHydro, onSourceChange);

  function select(key: any): reactiveObject<boolean> {
    arm();
    ...
    if (!derived) {
      ...
      if (fresh !== getValue(derived)) derived(fresh);
    }
    return derived;
  }
  select.dispose = () => {
...
const proxyHandler = {
  ...
  get(target, prop, receiver) {
    ...
    if (prop === Placeholder.observe) return bindToTarget(target, observeMethod);
    if (prop === Placeholder.getObservers)
      return bindToTarget(target, getObserversMethod);
    if (prop === Placeholder.unobserve) return bindToTarget(target, unobserveMethod);
