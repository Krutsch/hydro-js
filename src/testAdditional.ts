...
export function registerAdditionalTests(
  ...
  describe("shared coverage additions", () => {
    ...
    it("wires appended html rows without rescanning the full view root", async () => {
      ...
      try {
        view(
          "#viewAppendScopedScan",
          data,
          (_item: any, index: number) =>
            html`<li onclick=${() => clicks++}>
              ${data[index].id}${document.createTextNode("tail")}
            </li>`,
        );
        data([{ id: 1 }]);
        await sleep(5);
    ...
    it("reuses view rows with arbitrary enumerable fields", async () => {
      ...
      view(
        ...
        (_item: any, index: number) =>
          html`<li>
            ${data[index].id}/${data[index].label}/${data[index].extra}
          </li>`,
      );

      data([{ id: 2, label: "new", extra: "second" }]);
      await sleep(2);
      const updated = list.textContent.trim() === "2/new/second";
      unmount();
      unset(data);
      setReuseElements(true);
      return updated;
