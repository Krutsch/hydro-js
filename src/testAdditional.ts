...
export function registerAdditionalTests(
  ...
  describe("shared coverage additions", () => {
    it("owns rendered node teardown records", () => {
      const fragmentToElements = new WeakMap<DocumentFragment, Array<ChildNode>>();
    ...
    it("records update effects behind adapter seam", () => {
      const text = document.createTextNode("before");
      const changes = [[0, 6, undefined, {}, "value"]] as [
        number,
        number,
        string | undefined,
        object,
      ...
      const engine = createUpdateEngine({
        ...
        schedule: (fn: Function, ...args: any[]) => fn(...args),
        isAsync: () => false,
