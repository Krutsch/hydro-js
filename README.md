...
> A lightweight reactive UI library via template literal tags.<br> Support in all modern Browsers and with Server-Side Rendering!
...
returns: `(key: any) => ReturnType<typeof reactive<boolean>>` with a `dispose()` method

Fine-grained alternative to one ternary per row on a shared signal (like Solid's `createSelector`). A single subscription tracks the source and only the derived values whose membership flips are updated, so selecting a row costs O(1) observer work instead of O(rows). Compose the per-key value with `ternary()` for rendering.

Each derived value is owned like a ternary: `unset()` it when its row is cleaned up (this also removes it from the selector), and call the returned function's `dispose()` when the selector itself is no longer needed.
...
view("#list", data, (item, index) => {
  const matches = isSelected(item.id);
  const className = ternary((val) => val, "danger", "", matches);
  const row = html`<li class=${className}>${data[index].label}</li>`;
  onCleanup(unset, row, className);
  onCleanup(unset, row, matches);
  return row;
});
// Later: isSelected.dispose(); unset(selected);
```
