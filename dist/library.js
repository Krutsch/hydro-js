...
const allNodeChanges = new WeakMap(); // Each Node starts with one inline change and upgrades only when needed.
const elemEventFunctions = new WeakMap(); // Stores event functions in order to compare Elements against each other.
function addToOneOrMany(current, value, unique = false) {
    if (current === undefined)
        return value;
    if (Array.isArray(current)) {
        if (!unique || !current.includes(value))
            current.push(value);
        return current;
    }
    if (unique && current === value)
        return current;
    return [current, value];
}
function runOneOrMany(value) {
...
function bindEvent(node, attribute, value, previous, removeAttribute = true) {
...
function trackBoundElement(proxy, elem) {
    ...
    if (next !== current)
        boundElemProxies.set(elem, next);
}
...
function startsWithReactiveMarker(value) {
    return (value.startsWith("{{") ||
        (isServerSideCached && value.startsWith("hydro-reactive-" /* Placeholder.reactiveKey */)));
}
...
/* c8 ignore end */
function wireNewRows(elements, eventFunctions) {
    for (const element of elements) {
        if (isDocumentFragment(element)) {
            let child = element.firstChild;
            while (child) {
                setReactivity(child, eventFunctions);
                child = child.nextSibling;
            }
        }
        else {
            setReactivity(element, eventFunctions);
        }
    }
}
        ...
        if (isTextNode(node)) {
            ...
            if (key === "bind") {
                ...
                if (node instanceof window.HTMLSelectElement ||
                    ...
                    const isChecked = node instanceof window.HTMLInputElement &&
                        node.type === "checkbox" /* Placeholder.checkbox */;
                    const eventName = node instanceof window.HTMLTextAreaElement ||
                        (node instanceof window.HTMLInputElement &&
...
function writeTwoWayValue(node, value, initial) {
    if (node instanceof window.HTMLInputElement && node.type === "radio" /* Placeholder.radio */) {
...
function isNodeChangeList(changes) {
    return Array.isArray(changes[0]);
}
function addNodeChange(changes, change) {
...
function setTraces(start, end, node, hydroKey, resolvedObj, key) {
    ...
    allNodeChanges.set(node, changesForNode ? addNodeChange(changesForNode, change) : change);
...
function keysOf(value) {
    return Reflect.get(value, keysSymbol.description);
}
function reactiveParent(reactiveHydro) {
    const keys = keysOf(reactiveHydro);
    const lastProp = keys[keys.length - 1];
    const parent = keys.length === 1 ? hydro : resolveObject(keys)[1];
    return [lastProp, parent];
}
function unset(reactiveHydro) {
    ...
    const selectorOwner = selectorOwners.get(reactiveHydro);
    if (selectorOwner) {
        selectorOwner.map.delete(selectorOwner.key);
        selectorOwners.delete(reactiveHydro);
    }
...
// Ownership of per-key values created by `selector()`: unsetting one removes
// it from the selector so a cleared list releases every derived signal.
const selectorOwners = new WeakMap();
// Fine-grained alternative to one ternary per row on a shared signal (like
// Solid's `createSelector`). A single subscription tracks the source and only
// the derived values whose membership flips are updated, so selecting a row
// costs O(1) observer work instead of O(rows). Compose with `ternary()` for
// rendering and dispose each derived value with `unset()` like any ternary.
function selector(reactiveHydro) {
    const perKey = new Map();
    let current = getValue(reactiveHydro);
    let disposed = false;
    const onSourceChange = (newVal) => {
        if (newVal === null) {
            const prev = perKey.get(current);
            if (prev && getValue(prev))
                prev(false);
            current = null;
            return;
        }
        ...
        if (prev && getValue(prev))
            prev(false);
        const next = perKey.get(newVal);
        if (next && !getValue(next))
            next(true);
        current = newVal;
    };
    // Writing null to the source drops every observer (the library contract
    // ternaries rely on to dispose themselves), so the selector re-arms on
    // every lookup: Set semantics dedupe while the subscription is alive.
    const arm = () => {
        if (!disposed)
            observe(reactiveHydro, onSourceChange);
    };
    const stopObserving = observe(reactiveHydro, onSourceChange);
    function select(key) {
        arm();
        let derived = perKey.get(key);
        if (!derived) {
            derived = reactive(getValue(reactiveHydro) === key);
            perKey.set(key, derived);
            selectorOwners.set(derived, { map: perKey, key });
        }
        else {
            // The re-armed subscription only sees future writes, so heal a value
            // that went stale across a null write.
            const fresh = getValue(reactiveHydro) === key;
            if (fresh !== getValue(derived))
                derived(fresh);
        }
        return derived;
    }
...
const proxyHandler = {
    ...
    has(target, prop) {
        if (prop === "observe" /* Placeholder.observe */ ||
            prop === "getObservers" /* Placeholder.getObservers */ ||
            prop === "unobserve" /* Placeholder.unobserve */) {
            return true;
        }
        return Reflect.has(target, prop);
    },
    getOwnPropertyDescriptor(target, prop) {
        const method = prop === "observe" /* Placeholder.observe */
            ? observeMethod
            : prop === "getObservers" /* Placeholder.getObservers */
                ? getObserversMethod
                : prop === "unobserve" /* Placeholder.unobserve */
    ...
    get(target, prop, receiver) {
        ...
        if (prop === "getObservers" /* Placeholder.getObservers */)
            return bindToTarget(target, getObserversMethod);
        if (prop === "unobserve" /* Placeholder.unobserve */)
    ...
    const stopViewObserver = observe(data, (newData, oldData) => {
        ...
        try {
            ...
            if (oldData?.length &&
                ...
                newElements = slicedData.map((item, i) => renderFunction(item, i + length));
                newRowsAreWired =
