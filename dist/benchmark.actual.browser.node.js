...
function parseArgs(input) {
    ...
    for (let index = 0; index < input.length; index++) {
        ...
        if (argument === "--write-baseline")
            parsed.writeBaselinePath = value;
        if (argument === "--baseline")
            parsed.baselinePath = value;
        if (argument === "--profile")
            parsed.profileMode = value;
        if (argument === "--profile-output")
            parsed.profileOutput = value;
    }
    return parsed;
}
...
const maxRegressionPercent = Number(process.env.ACTUAL_MAX_REGRESSION_PERCENT ?? 15);
const minRegressionMs = Number(process.env.ACTUAL_MIN_REGRESSION_MS ?? 0.25);
const maxFrameRegressionPercent = Number(process.env.ACTUAL_MAX_FRAME_REGRESSION_PERCENT ?? 50);
const minFrameRegressionMs = Number(process.env.ACTUAL_MIN_FRAME_REGRESSION_MS ?? 10);
const operations = [
    "create",
    "replace",
    "create-many",
...
if (selectedOperations.length === 0) {
    throw new Error(`Unknown operation ${requestedOperation}. Expected one of ${operations.join(", ")}`);
}
if (profileMode &&
    profileMode !== "cpu" &&
    profileMode !== "allocation" &&
    profileMode !== "timeline") {
    throw new Error(`Unknown profile mode ${profileMode}. Expected cpu, allocation, or timeline`);
}
...
if (profileMode && (memoryMode || baselinePath || writeBaselinePath)) {
    throw new Error("--profile cannot be combined with --memory, --baseline, or --write-baseline");
}
const MIME = {
...
async function buildActualApp(minify) {
    await build({
...
async function measure(browser, port, operation) {
    ...
    try {
        await page.goto(`http://127.0.0.1:${port}/src/benchmark.actual.browser.html`, { waitUntil: "load" });
        await page.waitForFunction("window.__actualBenchmark && typeof window.__actualBenchmark.run === 'function'");
...
async function measureProfile(browser, port, operation, mode) {
    const context = await browser.newContext();
    const page = await context.newPage();
    ...
    try {
        await page.goto(`http://127.0.0.1:${port}/src/benchmark.actual.browser.html?profile=${mode}`, { waitUntil: "load" });
        await page.waitForFunction("window.__actualBenchmark && typeof window.__actualBenchmark.run === 'function'");
        ...
        if (!completed.stream)
            throw new Error("Tracing did not return a stream");
        let trace = "";
...
async function measureMemory(browser, port, cycles) {
    const context = await browser.newContext();
    const page = await context.newPage();
    const client = await context.newCDPSession(page);
    ...
    try {
        await page.goto(`http://127.0.0.1:${port}/src/benchmark.actual.browser.html`, { waitUntil: "load" });
        await page.waitForFunction("window.__actualBenchmark && typeof window.__actualBenchmark.memory === 'function'");
        ...
        await collectGarbage();
        const aliveAfterReturn = await page.evaluate(() => window.__actualBenchmark.memorySurvivors());
...
async function main() {
    ...
    const browser = await chromium.launch(memoryMode
    ...
    try {
        ...
        if (profileMode) {
            ...
            const { result, profile, trace } = await measureProfile(browser, port, operation, profileMode);
            const outputPath = resolve(rootDir, profileOutput);
            const traceOutput = profileMode === "timeline" ? outputPath : `${outputPath}.trace.json`;
            ...
            const bundle = await readFile(resolve(distDir, "benchmark.actual.app.js"));
            ...
            const processStatus = process.platform === "linux"
            ...
            const report = {
                ...
                config: {
                    ...
                    context: "fresh",
                    profileMode,
                    captureScope: "run including setup, handler, frames, and verification",
                    cpuSamplingIntervalUs: profileMode === "cpu" ? 1000 : undefined,
                    allocationSamplingIntervalBytes: profileMode === "allocation" ? 32768 : undefined,
                    includeCollectedObjects: profileMode === "allocation" ? true : undefined,
                },
        ...
        if (writeBaselinePath) {
            await writeFile(resolve(rootDir, writeBaselinePath), JSON.stringify(report, null, 2) + "\n");
        }
        if (baselinePath) {
            const baseline = JSON.parse(await readFile(resolve(rootDir, baselinePath), "utf8"));
            for (const key of [
                "fixture",
                "timing",
                "bundle",
                ...
                if (baseline.metadata?.[key] !== report.metadata[key]) {
                    failures.push(`baseline ${key} mismatch (baseline=${baseline.metadata?.[key]}, run=${report.metadata[key]})`);
                }
            }
            for (const key of ["repeats", "warmups"]) {
                if (baseline.config?.[key] !== report.config[key]) {
                    failures.push(`baseline ${key} mismatch (baseline=${baseline.config?.[key]}, run=${report.config[key]})`);
                }
            }
            const before = new Map(baseline.results.map((result) => [result.operation, result]));
            for (const result of results) {
                ...
                if (deltaMs > minRegressionMs && deltaPercent > maxRegressionPercent) {
                    failures.push(`${result.operation} median +${deltaPercent.toFixed(1)}% (> ${maxRegressionPercent}%)`);
                }
                const frameDeltaMs = result.frameMedianMs - baselineResult.frameMedianMs;
                const frameDeltaPercent = baselineResult.frameMedianMs
                    ? (frameDeltaMs / baselineResult.frameMedianMs) * 100
                    : frameDeltaMs > 0
                        ? Infinity
                        : 0;
                if (frameDeltaMs > minFrameRegressionMs &&
                    frameDeltaPercent > maxFrameRegressionPercent) {
                    failures.push(`${result.operation} frame median +${frameDeltaPercent.toFixed(1)}% (> ${maxFrameRegressionPercent}%)`);
                }
            }
            for (const result of baseline.results) {
                if (!results.some((current) => current.operation === result.operation)) {
