...
function rowIds() {
    return Array.from(rows(), (row) => row.querySelector("td")?.textContent ?? "");
}
function rowNodes() {
...
const profilingEnabled = new URLSearchParams(window.location.search).has("profile");
let memoryRefs;
function markProfile(name) {
    if (profilingEnabled)
...
async function settleForMemory() {
    ...
    if (typeof window.gc !== "function") {
        throw new Error("window.gc is unavailable; launch Chromium with exposed GC");
    }
    window.gc();
...
function verify(operation, beforeRows, beforeLabels) {
    ...
    const updatedLabels = beforeLabels.every((label, index) => {
        const afterLabel = afterRows[index]?.querySelector("td:nth-child(2)")?.textContent ?? "";
        return index % 10 === 0
            ? afterLabel === `${label} !!!`
            : afterLabel === label;
    });
    const passed = (operation === "create" && rowCount === 1000) ||
        (operation === "replace" && rowCount === 1000 && rowsReplaced) ||
        (operation === "create-many" && rowCount === 10000) ||
        (operation === "append" &&
            rowCount === beforeCount + 1000 &&
            rowsPreserved) ||
        (operation === "update" && rowCount === beforeCount && rowsPreserved && updatedLabels) ||
        (operation === "select" &&
            selectedCount() === 1 &&
...
export function installActualBenchmark() {
    window.__actualBenchmark = {
        async run(operation) {
            ...
            const beforeLabels = beforeRows.map((row) => row.querySelector("td:nth-child(2)")?.textContent ?? "");
        ...
        async memory(cycles) {
            ...
            for (let cycle = 0; cycle < cycles; cycle++) {
                ...
                button("clear").click();
                if (rows().length !== 0) {
                    throw new Error(`clear left ${rows().length} rows after cycle ${cycle}`);
                }
            }
            await settleForMemory();
            memoryRefs = refs;
            const aliveBeforeReturn = refs.reduce((count, ref) => count + (ref.deref() ? 1 : 0), 0);
            return {
                cycles,
                rowsPerCycle: 10000,
                aliveBeforeReturn,
        ...
        memorySurvivors() {
            if (!memoryRefs)
                throw new Error("memory probe has not run");
            return memoryRefs.reduce((count, ref) => count + (ref.deref() ? 1 : 0), 0);
        },
        releaseMemoryRefs() {
