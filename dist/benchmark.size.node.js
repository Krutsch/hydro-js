...
async function measureTarget(target) {
    const rawCode = target.type === "source"
        ? await readFile(target.path, "utf8")
        : await bundleTarget(target.entry, false);
    const minifiedCode = target.type === "source"
...
async function bundleTarget(entry, minify) {
    ...
    if (!output)
        throw new Error(`No esbuild output for ${entry}`);
    return output.text;
}
...
function findFailures(report, baseline, options) {
    const failures = [];
    const full = report.results.find((result) => result.name === "full");
    if (options.maxFullBrotli !== undefined &&
        full &&
    ...
    if (baseline && options.maxRegressionPercent !== undefined) {
        ...
        for (const result of report.results) {
            ...
            if (delta > allowed) {
                failures.push(`${result.name} brotli +${delta} exceeds ${options.maxRegressionPercent}%`);
            }
        }
    }
    return failures;
}
function createDeltas(report, baseline) {
    const baselineByName = new Map(baseline.results.map((result) => [result.name, result]));
    const deltas = {};
    for (const result of report.results) {
        const before = baselineByName.get(result.name);
...
function formatReport(report, baseline, failures) {
    const deltas = baseline ? createDeltas(report, baseline) : undefined;
    const lines = [];
    lines.push("");
    lines.push("hydro-js size benchmark");
    lines.push("=".repeat(86));
    lines.push(`${"target".padEnd(16)} ${"raw".padStart(9)} ${"min".padStart(9)} ${"gzip".padStart(9)} ${"brotli".padStart(9)} ${"delta br".padStart(10)}`);
    lines.push("-".repeat(86));
    for (const result of report.results) {
        const delta = deltas?.[result.name]?.brotliBytes;
        lines.push(`${result.name.padEnd(16)} ${formatBytes(result.rawBytes).padStart(9)} ${formatBytes(result.minifiedBytes).padStart(9)} ${formatBytes(result.gzipBytes).padStart(9)} ${formatBytes(result.brotliBytes).padStart(9)} ${formatDelta(delta).padStart(10)}`);
    }
    lines.push("=".repeat(86));
    if (failures.length) {
