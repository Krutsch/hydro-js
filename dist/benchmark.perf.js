...
const scaledInteractionOperations = [
    {
    ...
    },
    {
        ...
        verify: (app, rows, repetitions) => app.rowCount() === rows &&
            app.firstId() === "1" &&
...
export async function runScaledInteractionScenarios(deps = {}) {
    const config = {
    ...
    for (const [impl, createApp] of impls) {
        for (const operation of scaledInteractionOperations) {
            ...
            const samples = [];
            let ok = true;
            for (let iteration = 0; iteration < config.warmups + config.repeats; iteration++) {
                const app = createApp();
...
export function formatScaledInteractionReport(report) {
    const lines = [
        "",
        "hydro-js scaled interaction benchmark",
        "=".repeat(112),
        `${"impl".padEnd(9)} ${"operation".padEnd(32)} ${"rows".padStart(7)} ${"reps".padStart(6)} ${"total median".padStart(13)} ${"total min".padStart(11)} ${"per-op median".padStart(14)} ${"per-op min".padStart(12)} ${"spread".padStart(8)}  status`,
        "-".repeat(112),
    ];
    for (const result of report.results) {
        lines.push(`${result.impl.padEnd(9)} ${result.operation.padEnd(32)} ${String(result.rows).padStart(7)} ${String(result.repetitions).padStart(6)} ${formatMs(result.medianMs).padStart(13)} ${formatMs(result.minMs).padStart(11)} ${formatMs(result.perOperationMedianMs).padStart(14)} ${formatMs(result.perOperationMinMs).padStart(12)} ${`${result.spreadPct.toFixed(0)}%`.padStart(8)}  ${result.ok ? "ok" : "FAIL"}`);
    }
